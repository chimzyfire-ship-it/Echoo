...
export function OutingScreen({ initial, userId }: { initial?: string; userId?: string }) {
  ...
  return <ScrollView ref={scroll} style={styles.screen} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[styles.content, { paddingTop: Math.max(insets.top, 44) + 6, paddingBottom: Math.max(insets.bottom, 24) + 24 }]}>
