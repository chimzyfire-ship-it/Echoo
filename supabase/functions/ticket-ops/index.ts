...
async function loadOps(supabase: ReturnType<typeof getSupabaseAdmin>, eventId: string) {
  ...
  const [{ data: tiers, error: tiersError }, { data: orders, error: ordersError }, { data: tickets, error: ticketsError }, { data: holds, error: holdsError }] = await Promise.all([
    supabase.from("ticket_tiers").select("*").eq("event_id", eventId).order("sort_order", { ascending: true }),
    supabase
      .from("ticket_orders")
    ...
    supabase
      .from("ticket_items")
      .select("*, ticket_orders(buyer_name,buyer_email,status,total_cents,currency,created_at), ticket_tiers(name)")
      .eq("event_id", eventId)
      .order("created_at", { ascending: false })
      .limit(500),
    supabase
  ...
  const metrics = {
    ...
    pendingOrders: orderRows.filter((order: any) => order.status === "pending").length,
    revenueCents: orderRows
      .filter((order: any) => order.status === "confirmed")
      .reduce((sum: number, order: any) => sum + Number(order.total_cents || 0), 0),
  };

  const enrichedTiers = (tiers || []).map((tier: any) => {
    const confirmed = ticketRows.filter((ticket: any) => ticket.tier_id === tier.id).length;
    ...
    const pending = orderRows
      .filter((order: any) => order.tier_id === tier.id && order.status === "pending")
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "event-status") {
      ...
      if (!["draft", "published", "archived"].includes(status)) return jsonResponse({ error: "Invalid event status." }, 422);
      ...
      if (event.location_entity_id) {
        const locationStatus = status === "published" ? "published" : status === "archived" ? "archived" : "draft";
      ...
      if (!tierId || !["on_sale", "paused", "sold_out"].includes(saleStatus)) {
        return jsonResponse({ error: "tierId and valid saleStatus are required." }, 422);
      }
      ...
      const committed = Number(tier.confirmed || 0) + Number(tier.activeHeld || 0);
      if (capacity < committed) {
        return jsonResponse({ error: "Capacity cannot be below confirmed tickets plus active holds." }, 422);
      }
      const remaining = Math.max(0, capacity - committed);
      const { error } = await supabase
        .from("ticket_tiers")
        .update({ capacity, remaining_quantity: remaining, sale_status: saleStatus })
        .eq("id", tierId)
        .eq("event_id", eventId);
      if (error) throw error;
