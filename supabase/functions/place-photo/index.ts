...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey)
      return jsonResponse({ error: "Photo provider is unavailable" }, 503);
    const response = await fetch(
      `https://places.googleapis.com/v1/${payload.photoName}/media?maxWidthPx=${url.searchParams.get("surface") === "detail" ? 1200 : 720}`,
